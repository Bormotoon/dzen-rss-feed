...
- добавляет featured image первым HTML-блоком в `content:encoded`, если такой URL ещё не встречается в теле материала;
- вырезает блок источника Pedobraz и пустые Themify Builder wrappers из HTML фида;
- по настройке добавляет в конец материала отдельный абзац со ссылкой на публикацию на сайте педобраз.рф;
...
- featured image дублируется в тело статьи первым простым `p > img` блоком, если её URL ещё не встречается в контенте;
...
| `Добавлять ссылку на новость на сайте` | Добавляет в конец `content:encoded` отдельный абзац со ссылкой на публикацию на сайте педобраз.рф. |
...
- prepend featured image в начало `content:encoded`, если у записи есть post thumbnail и в теле ещё нет того же URL;
...
Плагин бесплатный. Если он избавил вас от ручной возни с лентой для Дзена — поддержите разработку:
