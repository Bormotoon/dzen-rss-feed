...
- держит WebP в медиатеке сайта, но автоматически транскодирует локальные unsupported images в JPEG для `enclosure`;
...
- локальные WebP-изображения автоматически транскодируются в JPEG только для RSS-обложки, чтобы не ломать экономный storage на сайте;
...
Если исходник лежит в WordPress как `webp`, плагин сначала пытается сделать JPEG-копию для RSS. Если конвертация не удалась, `enclosure` пропускается, но сам материал остаётся в ленте.

### Почему в RSS JPEG, а на сайте WebP?

Потому что это компромисс между совместимостью и экономией места:

- в медиатеке и на фронтенде сайт продолжает хранить WebP;
- для Дзена плагин генерирует отдельную JPEG-копию только для RSS;
- если локальная конвертация невозможна, пост не ломается, а просто идёт без `enclosure`.
