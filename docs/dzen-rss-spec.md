...
- Если attachment MIME не найден, плагин использует расширение `path` URL как резервный источник, поэтому `.webp` в URL path остаётся поддержанным даже без локального attachment mapping.
